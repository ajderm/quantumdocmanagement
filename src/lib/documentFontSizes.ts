...
export function roleForSize(px: number): FontSizeRole {
  ...
  if (px === 15 || px === 11) return 'header';
  if (px === 14 || px === 12) return 'body';
...
export const FONT_SIZE_ROLES: Array<{
  ...
  { key: 'header', label: 'Section Headers', description: 'Bold underlined section titles inside documents', sample: 15 },
  { key: 'body', label: 'Body Text', description: 'Paragraphs, signature labels, table column headers', sample: 14 },
  { key: 'table', label: 'Table Rows', description: 'Equipment/line-item rows, dealer header address lines', sample: 10 },
  { key: 'fine', label: 'Fine Print / T&C', description: 'Terms & conditions and footnotes', sample: 8 },
];
