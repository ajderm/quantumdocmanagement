          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="amount" value={formData.amount} onChange={(e) => handleAmountChange(e.target.value)} className="pl-7" placeholder="0.00" />
              </div>
              <p className="text-xs text-muted-foreground">Pre-filled from Quote Total Buyout</p>
            </div>
            <div className="space-y-2">
              <Label htmlFor="customerName">Customer Name</Label>
              <Input id="customerName" value={formData.customerName} onChange={(e) => updateField("customerName", e.target.value)} placeholder="Company legal name" />
              <p className="text-xs text-muted-foreground">Pre-filled from FMV Lease</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input id="leaseNumber" value={formData.leaseNumber} onChange={(e) => updateField("leaseNumber", e.target.value)} placeholder="e.g., ABC-12345" />
            </div>
            <div className="space-y-2">
              <Label>Lease End Date</Label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !formData.leaseEndDate && "text-muted-foreground")}>
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {formData.leaseEndDate ? format(formData.leaseEndDate, "MM/dd/yyyy") : "Select date"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar mode="single" selected={formData.leaseEndDate || undefined} onSelect={(date) => updateField("leaseEndDate", date || null)} initialFocus className="pointer-events-auto" />
                </PopoverContent>
              </Popover>
            </div>
            <div className="space-y-2">
              <Label htmlFor="leaseCompany">Lease Company</Label>
              <Input id="leaseCompany" value={formData.leaseCompany} onChange={(e) => updateField("leaseCompany", e.target.value)} placeholder="Leasing company name" />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="py-3">
          <CardTitle className="text-sm flex items-center justify-between">
            <span>Equipment ({formData.equipment.length}/{MAX_EQUIPMENT_ROWS})</span>
            <Button type="button" variant="outline" size="sm" onClick={addEquipmentRow} disabled={formData.equipment.length >= MAX_EQUIPMENT_ROWS}>
            ...
            {formData.equipment.map((item, index) => (
              <div key={item.id} className="grid grid-cols-12 gap-2">
                <div className="col-span-3">
                  <Input value={item.make} onChange={(e) => updateEquipmentItem(index, "make", e.target.value)} placeholder="Make" className="h-8 text-sm" />
                </div>
                <div className="col-span-4">
                  <Input value={item.model} onChange={(e) => updateEquipmentItem(index, "model", e.target.value)} placeholder="Model" className="h-8 text-sm" />
                </div>
                <div className="col-span-4">
                  <Input value={item.serial} onChange={(e) => updateEquipmentItem(index, "serial", e.target.value)} placeholder="Serial #" className="h-8 text-sm" />
                </div>
                <div className="col-span-1 flex justify-center">
                  <Button type="button" variant="ghost" size="sm" onClick={() => removeEquipmentRow(index)} disabled={formData.equipment.length <= 1} className="h-8 w-8 p-0">
