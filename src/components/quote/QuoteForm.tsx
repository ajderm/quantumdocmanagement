...
export interface QuoteFormData { quoteNumber: string; quoteDate: string; preparedBy: string; preparedByPhone: string; preparedByEmail: string; companyName: string; address: string; address2: string; city: string; state: string; zip: string; phone: string; lineItems: QuoteLineItem[]; retailPrice: number; cashDiscount: number; selectedTerms: number[]; serviceBaseRate: number; includedBWCopies: string; includedColorCopies: string; overageBWRate: string; overageColorRate: string; }
...
export function QuoteForm({ deal, company, lineItems, dealOwner, onFormChange }: QuoteFormProps) {
  const [formData, setFormData] = useState<QuoteFormData>({ quoteNumber: '', quoteDate: new Date().toISOString().split('T')[0], preparedBy: '', preparedByPhone: '', preparedByEmail: '', companyName: '', address: '', address2: '', city: '', state: '', zip: '', phone: '', lineItems: [], retailPrice: 0, cashDiscount: 0, selectedTerms: [36, 48, 60], serviceBaseRate: 0, includedBWCopies: '', includedColorCopies: '', overageBWRate: '', overageColorRate: '' });

  useEffect(() => {
    const totalPrice = lineItems.reduce((sum: number, item: any) => sum + (item.price * item.quantity), 0);
    setFormData(prev => ({ ...prev, quoteNumber: deal?.hsObjectId || '', preparedBy: dealOwner ? `${dealOwner.firstName || ''} ${dealOwner.lastName || ''}`.trim() : '', preparedByEmail: dealOwner?.email || '', preparedByPhone: dealOwner?.phone || '', companyName: company?.name || '', address: company?.address || '', address2: company?.address2 || '', city: company?.city || '', state: company?.state || '', zip: company?.zip || '', phone: company?.phone || '', lineItems: lineItems.map((item: any) => ({ id: item.id, quantity: item.quantity, model: item.model || item.sku || '', description: item.description || item.name || '', price: item.price })), retailPrice: totalPrice, cashDiscount: totalPrice * 0.95 }));
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          {formData.lineItems.map((item, idx) => (<div key={item.id} className="grid grid-cols-12 gap-2 items-center"><div className="col-span-1"><Input type="number" min="1" value={item.quantity} onChange={e => updateLineItem(idx, 'quantity', parseInt(e.target.value) || 1)} className="h-8 text-sm" /></div><div className="col-span-3"><Input value={item.model} onChange={e => updateLineItem(idx, 'model', e.target.value)} className="h-8 text-sm" /></div><div className="col-span-5"><Input value={item.description} onChange={e => updateLineItem(idx, 'description', e.target.value)} className="h-8 text-sm" /></div><div className="col-span-2"><Input type="number" min="0" step="0.01" value={item.price} onChange={e => updateLineItem(idx, 'price', parseFloat(e.target.value) || 0)} className="h-8 text-sm" /></div><div className="col-span-1"><Button type="button" variant="ghost" size="sm" onClick={() => removeLineItem(idx)} className="h-8 w-8 p-0 text-destructive"><Trash2 className="h-3.5 w-3.5" /></Button></div></div>))}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-3"><div><Label className="text-xs">Retail Price</Label><Input type="number" value={formData.retailPrice} onChange={e => updateField('retailPrice', parseFloat(e.target.value) || 0)} className="h-8 text-sm" /></div><div><Label className="text-xs">Cash Discount Price</Label><Input type="number" value={formData.cashDiscount} onChange={e => updateField('cashDiscount', parseFloat(e.target.value) || 0)} className="h-8 text-sm" /></div></div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><Label className="text-xs">Base Rate (per month)</Label><Input type="number" step="0.01" value={formData.serviceBaseRate} onChange={e => updateField('serviceBaseRate', parseFloat(e.target.value) || 0)} className="h-8 text-sm" /></div>
          <div className="grid grid-cols-2 gap-2"><div><Label className="text-xs">Included B/W</Label><Input value={formData.includedBWCopies} onChange={e => updateField('includedBWCopies', e.target.value)} className="h-8 text-sm" /></div><div><Label className="text-xs">Included Color</Label><Input value={formData.includedColorCopies} onChange={e => updateField('includedColorCopies', e.target.value)} className="h-8 text-sm" /></div></div>
          <div><Label className="text-xs">Overage B/W Rate</Label><Input value={formData.overageBWRate} onChange={e => updateField('overageBWRate', e.target.value)} className="h-8 text-sm" /></div>
          <div><Label className="text-xs">Overage Color Rate</Label><Input value={formData.overageColorRate} onChange={e => updateField('overageColorRate', e.target.value)} className="h-8 text-sm" /></div>
        </div>
      </div>
    </div>
  );
